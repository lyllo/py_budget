...
def init(input_file, output_file):
    ...
    if(toggle_db == "true"):
        print("\nIniciando 'load' do Sofisa investimentos em db...")
        timestamp = db.salva_registros(lista_de_registros, "CI Sofisa", os.path.basename(input_file))

    # Salva as informações em um arquivo Excel temporário
    if(toggle_temp_sheet == "true"):

        nome_arquivo = output_file
        print("\nIniciando 'load' do Sofisa investimentos em xlsx...")		
        files.salva_excel_temporario(nome_arquivo, "CI Sofisa", timestamp)